...
int loadFont(TTF_Font **police)
{
	...
	*police = TTF_OpenFont("../image/Ubuntu-Title.ttf", 20);
	return EXIT_SUCCESS;
}


void displayText(TTF_Font *police,Text *T,SDL_Surface *screen,perso pers,Background Bg,Uint32 dt)
{
	...
	//position
	sprintf(position,"pos x=%d pixels",pers.position.x);
	...
	//vitesse
	sprintf(vitesse,"vitesse= %f m/s",pers.velocity);
	...
	//acceleration
	sprintf(acceleration,"acceleration= %f m/s/s",pers.acceleration);
